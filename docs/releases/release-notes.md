...
- 章节编辑器里的 AI 修正预览现在更稳了。遇到模型返回较慢的整章修正或片段改写时，页面会继续等待候选方案生成，不会在结果还没回来时提前报“AI 修正失败”。
...
- 作品列表和发布详情会持续显示远端同步时间、已发布章节数、草稿箱状态和最近任务状态，便于续发和人工核对当前发布进度。
- 左侧导航结构已按工作区重新归位。现在“发布”作为独立分组放在“资产”和“系统”之间，分组下只保留“账号管理”和“作品列表”，发布详情从作品列表继续进入，菜单层级更符合实际操作路径。
- 当发布账号在远端已失效时，“同步远端进度”现在会直接提示需要重新扫码，不再只返回通用失败。同步失败后，账号状态也会立即刷新成“需要重新扫码”，方便用户直接回到账号管理修复。
- 发布进度统计现在更贴近真实数据。远端只返回最近一段已发布章节时，系统会按最高已发布章节号识别整体进度，不再把已经发布到第 180 章的作品误显示成 21 章或 0 章。
- 本地可发布章节数量也改成按实际正文章节统计，不再受旧工作流状态字段影响。像历史章节仍停留在“待修复”之类旧状态时，只要正文已经存在，就会计入本地已完成章节数。
...
- 发布详情页现在会把“待提交章节”和“已在平台存在的章节”分开展示。同步远端后发现已经在草稿箱或已发布的章节，不会再混在待提交列表里干扰人工判断。
- 如果当前计划里的章节只是因为同步远端进度才显示成“草稿箱”或“已发布”，现在仍然可以清除这份本地计划；真正已经由本地提交到平台的计划，仍会继续被保护，避免误删真实发布记录。
- 按计划开始发布时，系统现在只会继续提交还需要处理的章节。已经在平台存在的章节不会被再次送到接口，减少重复进草稿箱或重复发布的干扰。
- 发布计划表单现在拆成“立即发布”和“定时发布”两种方式。立即发布时不再要求填写日期时间，但仍需输入参与发布章节数量，默认值为 1；定时发布时则改为明确填写起始日期、发布时间和每日发布章节数。
- 定时发布生成计划时，起始日期现在会优先接在当前最后一个已占用计划日期之后，而不是总从当前日期重新起算，续发时更容易接上已有排期。
- 同步远端进度时，现在会把番茄端已经排好的发布时间一起带回本地。发布详情页会显示“远端已排期至”，新的定时发布计划也会优先从这个远端排期之后继续生成，不再只依赖本地已有计划。
- 点击“同步远端进度”后，发布详情页现在会立刻刷新当前页里的计划起始日期，不需要再手动刷新页面或重新进入详情页，续发时间可以直接接上远端最新排期。
- 发布平台如果把排期时间放在已发布章节里，并且使用 unix 时间戳返回，系统现在也会正确识别并换算成本地排期时间。像平台已排期到 2026-05-15 的作品，新的默认起始日期会继续落到 2026-05-16。
- 同步远端进度时，如果章节已经在平台侧进入草稿箱或完成发布，本地对应的发布任务状态现在也会一起收敛，不再长期卡在“排队中”或“提交中”。
- 远端同步时现在会忽略没有章节号、没有定时信息、标题仅为“未命名草稿”的占位草稿，减少这些临时草稿把本地草稿数和待发布判断拉偏。
- 桌面端主工作区不再整页滚动。现在只有侧边菜单和内容区各自滚动，长菜单会在导航栏内部滚动，不会把整页一起带着移动。
- 左侧“小说列表”菜单项恢复为正常导航态。未选中时不再出现异常高亮，只有真正进入小说列表后才显示选中样式。
...
- 计费管理里的模型价格可以从系统设置中已启用的供应商模型列表新增。管理员不需要手工输入供应商和模型名称，选择可用模型后填写输入、输出和缓存命中价格即可启用扣费。

### 2026-05-06

- 登录页现在可以真正找回密码了。用户提交注册邮箱后会收到重置入口邮件，打开邮件链接就能设置新密码；重置成功后旧登录态会失效，继续使用时需要用新密码重新登录。
- 忘记密码和重置密码页面已经接入真实表单，不再只是占位说明。邮箱不存在时也会返回统一提示，避免暴露账号是否注册。
- 管理员现在可以在计费管理里统一设置模型输入、输出和缓存命中价格，并创建总额度套餐和包月套餐模板。包月套餐支持设置每日额度和 30 日有效期，后续兑换会按套餐规则直接发放到用户账户。
- 系统新增兑换码发放链路。管理员可以按套餐模板批量生成兑换码、查看当前状态并手动停用；用户在钱包页输入兑换码后，可以兑换余额或套餐额度。
- 钱包页现在会显示当前可用套餐、模型价格和最近 30 天的使用趋势。金额消耗与 Token 消耗会按天聚合展示，包月额度按 UTC+8 每天刷新，并在模型调用时优先扣减当日套餐额度，再扣钱包余额。
- 计费管理页打开后不再白屏。页面加载前会先使用稳定的空数据占位，避免接口未返回时触发渲染异常。
- 钱包和计费相关服务在容器启动时会自动执行数据库迁移，线上部署后会先补齐计费表结构，再启动 API。
- 线上计费和钱包所需的数据表会随发布镜像一起交付，打开计费管理或钱包时不再因为缺少计费表而报错。

### 2026-05-02

- 任务中心默认会先聚焦还需要处理的任务。首次进入时不会再把已完成和已取消任务一并铺满列表，只有在切到已完成或已取消筛选时，才会专门查看这些历史结果。
- 已完成和已取消任务现在支持一键批量归档。进入对应筛选后，可以直接把当前列表里的已完成和已取消项一起收起，减少历史任务长期堆在任务中心里影响排查。
- 导演跟进红标改成只统计真正需要介入的部分。侧边栏和相关入口只会把待审批与异常项计入提醒，普通完成项不会再把红标数字抬高。

### 2026-04-29

- 服务重启后的自动导演恢复会控制高内存任务并发。节奏拆章、章节细化、章节执行和质量修复等阶段不会在启动时被多本书同时拉起，额外任务会暂停等待继续，减少启动内存占用过高和频繁 GC。
