...
* 当前 dispatch API 文档没有明确提供“按账号拉取番茄书籍列表”或“按书籍查询远端章节发布进度”的可靠接口；MVP 需要优先保证本地可维护、可续排、可去重。
...
生成计划前必须先检查当前平台绑定在本地已知的发布进度，并据此续排：

* 已进入草稿箱或已发布的章节，不得再次生成重复待提交流程项。
* 如果已有未完成计划，新的发布时间表应从最后一个已占用发布时间之后继续排，不覆盖已提交章节。
* 续排逻辑至少应基于本地已提交 job、章节状态和计划项完成去重；如果未来外部 API 补齐书籍级进度接口，再在此基础上增强。
...
* 只保留下拉框选择小说，不要在下方重复展示滚动小说卡片列表。
* 选中小说后，在内容区展示该小说的书名、简介、已有章节数量、预期章节数量，帮助用户确认当前发布对象。
...
* [ ] PRD 明确发布时间表在生成前要基于已有进度续排并去重。
* [ ] PRD 明确发布平台页的小说选择区不展示滚动卡片，只展示下拉和当前小说摘要。
