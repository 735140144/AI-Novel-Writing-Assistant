...
管理员现在可以设置模型输入、输出和缓存命中价格，创建总额度套餐和包月套餐模板，并生成、查看、停用兑换码。

钱包页现在可以查看当前套餐、模型价格和最近 30 天的消耗趋势；包月套餐会按 UTC+8 每天刷新额度，模型使用会先扣套餐每日额度，再扣钱包余额，额度用尽后会停止继续使用模型。

计费管理页打开时现在会先稳定展示空状态，不会再因为接口数据尚未返回而直接白屏。线上服务启动时也会先自动补齐计费相关数据表，再提供钱包和计费管理能力。

线上计费和钱包需要的数据表会随发布镜像一起部署，打开计费管理或钱包时不再因为缺少计费表而报错。
