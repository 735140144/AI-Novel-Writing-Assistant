...
小说工作台新增发布平台能力。现在可以绑定番茄账号和平台书籍，按整本书生成发布时间表，并把章节提交到草稿箱或正式发布。

菜单新增“发布平台”入口。进入后先选择小说，再完成账号绑定、书籍绑定和发布时间表。

发布计划支持“每日 8 点发布 2 章”这类自然语言节奏。系统会把每章计划时间落成 `YYYY-MM-DD HH:mm`，相同时间的章节合并提交，不同时间的章节自动拆分为不同发布任务。

章节发布状态现在能在本项目内持续追踪。未提交时显示未发布，提交到平台草稿箱后显示草稿箱，完成发布后显示已发布；提交失败或账号需要重新扫码时也会给出恢复入口。

发布平台里的番茄账号在扫码登录成功后会自动收起二维码，并把账号标签同步成当前番茄笔名，减少重复确认和手工维护。

绑定番茄书籍时会优先使用本地已维护书籍下拉。选择后会自动带出番茄书名和书籍编号，不再要求先理解原始番茄书籍 ID。

发布平台页的小说选择区改成下拉直选，去掉了重复滚动卡片；选中后会直接显示书名、简介、已有章节数量和预期章节数量。

生成发布时间表前会先接续当前已知发布进度。已经进入草稿箱、已发布或已有排期的章节会自动跳过，新的排期会从最后一个已占用发布时间之后继续生成，避免重复提交。

发布平台账号、小说、知识库、世界观、任务和模型配置继续按登录用户隔离。其他用户无法读取或复用不属于自己的小说、平台凭据和创作资产。

小说资产生成链路开始支持外部知识来源辅助。世界观、人物、故事线、写法和补充创作资产可以由外部来源生成候选，再由本项目保存为可追溯、可编辑、可复用的本地资产。
