...
AI 主驾自动执行章节时，如果某一章已经完成一次自动修复但仍低于质量阈值，系统会记录提醒并继续下一章，不会在同一章反复启动质量修复。章节流水线会同步更新章节完成或待修复状态，避免自动继续时把已审校章节误判为仍在生成中；同时会按模型路由选择写作、审校、修复和事实提取模型，任务详情会按当前阶段显示最新路由模型。
