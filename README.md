...
发布模块现在拆成了独立的菜单工作区。进入“发布”后，可以分别在“账号管理”里处理番茄账号，在“作品列表”里管理“小说 x 账号”绑定，再进入“发布详情”处理单本书的发布计划与状态。

账号管理会在扫码登录成功后自动收起二维码，并把账号标签同步成当前番茄笔名，减少重复维护账号名称。

作品列表改成表格视图，每条记录只代表一本小说绑定一个发布账号；同一本小说绑定多个账号时，会分成多条发布记录分别管理。

新增“添加作品”绑定流程。现在可以直接从现有小说中选择作品，再从本地已维护书籍下拉里选择对应番茄书籍，不需要手动理解或填写番茄书籍 ID。

发布详情页补齐了完整工作流：先同步远端进度，再设置参与发布章节数量，生成发布时间表后按计划顺序逐章提交。每次调用只提交一章，前一章失败时后续章节不会继续提交。

生成发布时间表前现在必须先手动同步远端进度。系统会读取番茄端已发布章节和有效草稿章节，自动跳过这些章节，并从已有排期之后继续生成新的计划，避免重复进入草稿箱或重复发布。

左侧导航结构也已经按工作区重新归位。“发布”现在作为独立分组放在“资产”和“系统”之间，分组下只保留“账号管理”和“作品列表”，发布详情从作品列表继续进入。

当发布账号在远端已失效时，“同步远端进度”现在会直接提示需要重新扫码，不再只返回通用失败；失败后账号状态也会立即刷新成“需要重新扫码”。

发布进度统计现在更贴近真实数据。远端只返回最近一段已发布章节时，系统会按最高已发布章节号识别整体进度，不再把已经发布到第 180 章的作品误显示成 21 章或 0 章；本地已完成章节数也改成按实际正文统计，不再被旧工作流状态压低。

右上角的错误提示不再常驻；普通报错提示会在 3 秒后自动关闭，减少连续操作时被旧错误信息长期占位。

当当前章节已经被本地发布时间表占用时，发布详情页现在可以直接清除“尚未真正提交到平台”的本地计划，再重新生成新的发布时间表；已经进入平台提交、草稿箱或已发布的计划仍会被拦住，避免误清真实发布进度。

设置与运行体验继续收拢到 Web / 内网部署主线。桌面端打包、更新和安装器相关入口从当前开发分支移除，避免后续发布链路继续维护两套运行形态。
