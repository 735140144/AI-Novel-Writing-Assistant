...
- 概要：已在 AGENTS.md 写入 beta 预发分支规则，明确功能分支先进入 beta 做预发验证，再回到 main。

计划清单：
- [ ] 确认 AGENTS.md 现有开发分支、beta 预发分支与发布规则之间的关系
- [ ] 新增 beta 预发分支工作流，明确功能分支、自测、预发集成、发布回 main 的顺序
- [ ] 补充发布前必须基于 beta 验证稳定的约束

进度记录：
- 2026-04-25 00:35 [已完成] 已完成 AGENTS.md 分支规则更新：功能分支先入 beta 预发验证，再合入 main 发布。
<!-- task-md-sync:item:task-a9206e4ae1:end -->
<!-- task-md-sync:item:task-3ae84511d6:start -->
### 角色库与小说角色同步系统
- 标识：`task-3ae84511d6`
- 状态：已完成
- 最近更新：2026-05-07 11:44
- 概要：角色库与小说角色同步系统当前阶段已归档为长期方案与首轮后端闭环；角色叙事岗位、关系张力和章节角色上下文包转入后续专题，不再作为当前活跃任务展开。
...
- 2026-05-07 11:44 [已完成] 当前阶段已归档，后续角色能力深化不再压在本任务下展开。
<!-- task-md-sync:item:task-3ae84511d6:end -->
<!-- task-md-sync:item:task-45bfc7e030:start -->
### 对象外部知识库绑定与自动生成
- 标识：`task-45bfc7e030`
- 状态：已计划
- 最近更新：2026-05-07 11:43
- 概要：为小说绑定外部知识库生成源，让世界观、人物基础信息、写法、反 AI、文笔与补充创作资产由外部知识库辅助自动生成，并在本项目内落成可追溯、可编辑、可复用的本地资产。

计划清单：
- [ ] 确认外部知识库生成资产范围与本项目落库边界
- [ ] 整理世界观、人物、写法、反 AI、文笔及补充资产的 spec
- [ ] 约定跨系统交互合同，明确上游需要提供的结构化能力
- [ ] 设计本项目侧绑定、生成、版本、溯源与审核流程
- [ ] 完成最小闭环验证方案

进度记录：
- 2026-05-07 11:43 [已计划] 已归档当前讨论焦点，切换到外部知识库绑定与自动生成 spec 草案阶段。
<!-- task-md-sync:item:task-45bfc7e030:end -->
<!-- task-md-sync:end -->
